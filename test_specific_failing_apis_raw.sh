...
SUPER_TOKEN=$(echo "$login" r '.token')
...
SCHOOL_ID=$(echo "$school" r '.school.id')
SUBDOMAIN=$(echo "$school" r '.tenant.subdomain')
ADMIN_EMAIL=$(echo "$school" r '.tenant.admin_credentials.email')
ADMIN_PASSWORD=$(echo "$school" r '.tenant.admin_credentials.password')

admin_login=$(curl -s -X POST "$BASE_URL/auth/login" -H "Content-Type: application/json" -H "X-Subdomain: $SUBDOMAIN" \
  -d "{\"email\":\"$ADMIN_EMAIL\",\"password\":\"$ADMIN_PASSWORD\"}")
ADMIN_TOKEN=$(echo "$admin_login" r '.token')

ACADEMIC_YEAR_ID=$(curl -s "$BASE_URL/academic-years" -H "Authorization: Bearer $ADMIN_TOKEN" -H "X-Subdomain: $SUBDOMAIN" r '.[0].id')
TERM_ID=$(curl -s "$BASE_URL/terms" -H "Authorization: Bearer $ADMIN_TOKEN" -H "X-Subdomain: $SUBDOMAIN" r '.[0].id')
...
STUDENT_ID=$(echo "$student" r '.student.id // .id')
