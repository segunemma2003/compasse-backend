...
echo "🚀 TESTING API ROUTES WITH WORKING AUTHENTICATION\n";
echo str_repeat("=", 60) . "\n\n";
...
// Step 1: Create a working authentication token using Laravel Tinker
echo "🔐 Step 1: Creating Authentication Token...\n";

// Use Laravel's built-in token creation
$createTokenScript = "
use App\Models\User;
...
$token = trim($tokenOutput);

if ($token && strpos($token, 'Token: ') === 0) {
    $token = trim(str_replace('Token: ', '', $token));
...
// Summary
echo str_repeat("=", 60) . "\n";
echo "📊 API ROUTE TESTING WITH WORKING AUTHENTICATION SUMMARY\n";
echo str_repeat("=", 60) . "\n";
...
echo "✅ Public Routes: " . (($results[1]['status'] == 200 || $results[1]['status'] == 401) ? "ACCESSIBLE" : "FAILED") . "\n";
