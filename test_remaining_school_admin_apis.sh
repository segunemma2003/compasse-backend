...
test_api "List Question Banks" "GET" "$BASE_URL/question-bank"
qb_create=$(test_api "Create Question Bank" "POST" "$BASE_URL/question-bank" "{\"subject_id\":$SUBJECT_ID,\"class_id\":$CLASS_ID,\"title\":\"Test QB\",\"description\":\"Test questions\"}")
QB_ID=$(echo "$qb_create" | jq -r '.question_bank.id // .data.id // .id' 2>/dev/null)
if [ -n "$QB_ID" ] && [ "$QB_ID" != "null" ]; then
  test_api "Get Question Bank" "GET" "$BASE_URL/question-bank/$QB_ID"
fi
...
# 9. EVENTS
echo -e "\n${CYAN}=== 9. EVENTS ===${NC}"
test_api "List Events" "GET" "$BASE_URL/events/events"
